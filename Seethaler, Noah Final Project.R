...
#test
data <- read.csv("C:/Users/noahs/OneDrive/Desktop/Fall 2024/BS806 Multivariable Analysis/Homework/Homework 2/gas_vapor.csv")
z <- data %>% select(y, x1, x2, x3, x4)
y <- z %>% select(y)
x <- z %>% select(-y)
...
data2 <- read.csv("C:/Users/noahs/OneDrive/Desktop/Fall 2024/BS806 Multivariable Analysis/Homework/Homework 4/depress2.csv")
z2 <- data2 %>% select(cesd, age, publicassist)
y2 <- z2 %>% select(cesd)
x2 <- z2 %>% select(-cesd)

proj(z2, y2, x2, instructions = F)
...
test2 <- lm(cesd ~ age + publicassist, data2)
summary(test2)
